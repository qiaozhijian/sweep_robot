...
void MyRobot::HandleRosbag(rosbag::MessageInstance m) {
    ...
    if (s2 != nullptr && m.getTopic() == topic_imu) {
        ...
        //odometryFile<<to_string(ros::Time().now().toSec())<<" "<<to_string(gyro_x)<<" "<<to_string(gyro_y)<<" "<<to_string(gyro_z)<<" "<<to_string(accel_x)<<" "<<to_string(accel_y)<<" "<<to_string(accel_z)<<" "<<to_string(pulseLeft)<<" "<<to_string(pulseRight)<<endl;
